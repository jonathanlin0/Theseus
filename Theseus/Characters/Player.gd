...
func _physics_process(delta):
	...
	if enemies_in_area.size() > 0:
		...
		for enemy in enemies_in_area:
			...
			new_collision_shape.rotation = angle - (PI/2)
			
			var enemy_vision_overlapping_bodies = new_area.get_overlapping_bodies()
